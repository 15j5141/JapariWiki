...
function checkBeforeSavingPage(rawText) {
  return new Promise(function(resolve, reject) {
    ...
    // fixme コメントidの重複確認
    checkBSP_NewCommentForm(resultText)
      .then(result => {
        resolve(result);
      })
      .catch(err => {
...
function checkComment(rawText, ncmb) {
  return new Promise(function(resolve, reject) {
    ...
    const promises = ids.map(id => ncmbC.getComment(Comment, id)); // idsからPromise作成.
    Promise.all(promises)
      .then(function(cforms) {
        ...
        Object.keys(commentLists).forEach(function(key) {
          const html =
            ...
            commentLists[key]
              .map(
                v =>
                  `<li>${replaceSyntax(v.contributor)}: ${replaceSyntax(
                    v.content
                  )}</li>`
              )
              .reduce(
...
function rndStr(length) {
  const moji = 'abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789';
  let result = '';
  let i = 0;
  while (i < length) {
